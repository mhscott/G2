function [plt, pltel] = modifiedNR(mod, control, plotdofs, plotelem )
% MODIFIEDNR Nonlinear analysis of model with modified Newton-Raphson procedure
% 	[PLT, PLTEL]=MODIFIEDNR(MOD,CONTROL,PLOTDOFS, PLOTELEM) Nonlinear analysis of 
% 	a model using the modified Newton-Raphson strategy under load control.
...
%	Low, Upp	= upper and lower factors of Kf
%
...
for loadstep = 1:nsteps
	...
	% Perform linearization at the begining of load step
	Kf(:) = 0;
	for i=1:nelem
		[id xyz ue] = localize( mod, i, U );
	...
	% LU factorization of stiffness matrix (once for load step)
	[ Low Upp ] = lu( Kf(free,free) );
	
	% Perform equilibrium iterations for load step
	
	for i=1:maxiter
		
		% Loop over elements to assemble restoring force only
		...
		% Back substitute factorized equations
		U(free,3) = Upp \ ( Low \ Pr );
